...
describe("generateContextSummary", () => {
  ...
  it("recency-first: newer memory appears first", () => {
    const s = new MemoryStore();
    s.add({
      ...mem, id: "old", name: "Old", content: "Old content", priority: 9, access_count: 100,
      created_at: "2026-01-01T00:00:00.000Z", last_accessed: "2026-01-01T00:00:00.000Z",
    });
    s.add({
      ...mem, id: "new", name: "New", content: "New content", priority: 1, access_count: 0,
  ...
  it("dedup skips similar entries", () => {
    const s = new MemoryStore();
    s.add({ ...mem, id: "a", name: "Report A", content: "TypeScript React frontend JWT auth", category: "decision-log", priority: 7, created_at: now, access_count: 5 });
    s.add({ ...mem, id: "b", name: "Report B", content: "TypeScript React frontend", category: "decision-log", priority: 5, created_at: now, access_count: 3 });
  ...
  it("newer beats older regardless of category", () => {
    ...
    s.add({ ...mem, id: "old-dl", name: "Old Decision", content: "Old decision log", category: "decision-log", priority: 9, created_at: yesterday, access_count: 0, last_accessed: yesterday });
    s.add({ ...mem, id: "new-up", name: "New Preference", content: "New user preference", category: "user-preference", priority: 1, created_at: now, access_count: 0, last_accessed: now });
    const summary = generateContextSummary(s, 1);
  ...
  it("evergreen: priority=10 force-included", () => {
    ...
    s.add({ ...mem, id: "ever", name: "Evergreen", content: "Critical evergreen fact", category: "general", priority: 10, created_at: veryOld, access_count: 0, last_accessed: veryOld });
    s.add({ ...mem, id: "new", name: "New Memory", content: "Newly stored", category: "user-preference", priority: 5, created_at: now, access_count: 0, last_accessed: now });
    expect(generateContextSummary(s, 2)).toContain("Evergreen");
  });

  it("session-transcript excluded from context", () => {
    const s = new MemoryStore();
    s.add({ ...mem, id: "dl", name: "Decision log", content: "Important decision", category: "decision-log", priority: 5, created_at: now, access_count: 0 });
    s.add({ ...mem, id: "tx", name: "Transcript", content: "Raw transcript dump", category: "session-transcript", priority: 9, created_at: now, access_count: 0 });
    const summary = generateContextSummary(s, 2);
  ...
  it("all-transcript store shows welcome", () => {
    const s = new MemoryStore();
    s.add({ ...mem, id: "tx", name: "Transcript", content: "Raw transcript", category: "session-transcript", priority: 9, access_count: 100, created_at: now });
  ...
  it("redacts private keys", () => {
    ...
    s.add({
      ...
      category: "project-context", priority: 9, access_count: 100, created_at: now, last_accessed: now,
    });
    const summary = generateContextSummary(s, 1);
