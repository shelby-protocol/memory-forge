...
describe("rulesToMemories", () => {
    ...
    const rules: ImportedRule[] = [
      {
        ...
        key: "test",
        content: content(
          "TypeScript 7.2 strict mode with exactOptionalPropertyTypes enabled for all compilation units across the monorepo",
        ),
      },
    ];
    ...
    const rules: ImportedRule[] = [
      {
        ...
        key: "test",
        content: content(
          "All database migrations must include down scripts for rollback support in all production and staging environments",
        ),
      },
    ];
    ...
    const rules: ImportedRule[] = [
      {
        ...
        key: "test",
        content: content(
          "GraphQL resolvers must implement DataLoader batching by default for all entity query operations and nested field resolution",
        ),
      },
    ];
  ...
  it("duplicate rule content deduplicated", () => {
    const dupContent = content(
      "Kyber-1024 lattice-based post-quantum cryptography implementation guidelines for secure protocol design and key exchange mechanisms",
    );
    const rules: ImportedRule[] = [
  ...
  it("dedup against existing saved memory", () => {
    const dedupContent = content(
      "Always use pnpm v10 instead of npm for monorepo package management across all workspaces and CI pipelines",
    );
    const mem: Memory = {
  ...
  it("new unique rule passes dedup against saved memories", () => {
    const baselineContent = content(
      "Xylophone zebra quantum cryptography baseline reference memory for import dedup verification against stored datasets",
    );
    const mem: Memory = {
    ...
    const rules: ImportedRule[] = [
      {
        ...
        key: "cursor-testing",
        content: content(
          "All API endpoints must return RFC 9457 Problem Details JSON error bodies with proper content negotiation headers for client error handling",
        ),
      },
    ];
    expect(rulesToMemories(rules)).toHaveLength(1);
  ...
  it("tags derived from source filename", () => {
    const rules: ImportedRule[] = [
      {
        source: "/Users/test/CLAUDE.md",
        key: "r1-" + Date.now(),
        content: content(
          "Specific rule content for verifying tag derivation from source filenames in the import pipeline with various edge cases and encoding scenarios",
        ),
      },
    ];
    ...
    const rules: ImportedRule[] = [
      {
        ...
        key: "c1",
        content: content(
          "Nix flakes for reproducible development environments across all machines operating systems and CPU architectures",
        ),
      },
      {
        ...
        key: "c2",
        content: content(
          "OpenTelemetry tracing required for all gRPC service-to-service calls with W3C context propagation and sampling policies",
        ),
      },
    ];
    expect(rulesToMemories(rules)).toHaveLength(2);
