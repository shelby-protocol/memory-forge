...
t("corrupted file does not crash loader", () => {
  loadAllMemories();
});
...
s2.add(dup1);
s2.add(dup2);
t("both stored (different IDs)", () => {
  if (s2.size() !== 2) throw new Error("size=" + s2.size());
});
const overlap = contentOverlap(dup1.content, dup2.content);
t("100% overlap detected", () => {
  if (overlap < 0.99) throw new Error("overlap=" + overlap);
});
...
t("create", () => {
  if (!s3.get(lifeId)) throw new Error("not found");
});
...
m.priority = 9;
t("update content", () => {
  if (s3.get(lifeId)!.content !== "Updated content v2") throw new Error("content");
});
t("update category", () => {
  if (s3.get(lifeId)!.category !== "decision-log") throw new Error("category");
});
t("update tags", () => {
  if (s3.get(lifeId)!.tags.length !== 2) throw new Error("tags");
});
t("update priority", () => {
  if (s3.get(lifeId)!.priority !== 9) throw new Error("priority");
});
t("update name", () => {
  if (s3.get(lifeId)!.name !== "Updated content v2") throw new Error("name");
});
const found = s3.keywordSearch("updated", { limit: 5 });
t("search finds updated", () => {
  if (found.length === 0) throw new Error("not found");
});
t("search score reasonable", () => {
  if (found[0]._score! < 5) throw new Error("score=" + found[0]._score);
});
s3.remove(lifeId);
t("forget removes", () => {
  if (s3.get(lifeId) !== null) throw new Error("still exists");
});
...
t("high priority wins", () => {
  if (!summary.includes("High Priority")) throw new Error("low won");
});
t("low priority excluded", () => {
  if (summary.includes("Low Priority")) throw new Error("low included");
});
...
t("last write wins", () => {
  if (s5.get(cid)!.content !== "Second write") throw new Error(s5.get(cid)!.content);
});
...
t("round-trip content", () => {
  if (imported.get("e1")!.content !== "Data for export") throw new Error("lost");
});
t("round-trip tags", () => {
  if (imported.get("e1")!.tags[0] !== "export") throw new Error("tags lost");
});
...
t("single tag", () => {
  if (s7.list({ tags: ["react"], limit: 10, offset: 0 }).length !== 2) throw new Error("react");
});
t("OR multi-tag", () => {
  if (s7.list({ tags: ["python", "django"], limit: 10, offset: 0 }).length !== 1) throw new Error("python");
});
t("no-match tag", () => {
  if (s7.list({ tags: ["rust"], limit: 10, offset: 0 }).length !== 0) throw new Error("rust");
});
...
for (let i = 0; i < 500; i++) {
  s8.add({
    id: `perf-${i}`,
    name: `Memory ${i}`,
    content: `Content for memory number ${i} with tags and metadata`,
...
t("500 inserts under 500ms", () => {
  if (insertTime > 500) throw new Error(insertTime + "ms");
});

const searchStart = Date.now();
for (let i = 0; i < 50; i++) s8.keywordSearch(`content ${i}`, { limit: 10 });
t("50 searches under 200ms", () => {
  if (Date.now() - searchStart > 200) throw new Error(Date.now() - searchStart + "ms");
});
...
t("empty store list", () => {
  if (s9.list({ limit: 10, offset: 0 }).length !== 0) throw new Error("not empty");
});
t("empty store size 0", () => {
  if (s9.size() !== 0) throw new Error("not zero");
});
t("empty store stats", () => {
  const st = s9.stats();
  if (st.total !== 0 || st.total_accesses !== 0) throw new Error("not empty stats");
});
t("empty store keyword search", () => {
  if (s9.keywordSearch("anything", { limit: 5 }).length !== 0) throw new Error("found in empty");
});
t("empty store get nonexistent", () => {
  if (s9.get("nope") !== null) throw new Error("found in empty");
});
...
t("LRU capped at 5000", () => {
  if (s10.size() > 5000) throw new Error("size=" + s10.size());
});
